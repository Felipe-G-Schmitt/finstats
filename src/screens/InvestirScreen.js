...
export default function InvestirScreen({ navigation }) {
  ...
  const grupos = useMemo(() => {
    ...
    const porMes = {};
    for (const inv of ordenados) { const ym = mesDe(inv.data); (porMes[ym] = porMes[ym] || []).push(inv); }
    return Object.keys(porMes).sort((a, b) => (a < b ? 1 : -1)).map((ym) => ({
      ym, total: porMes[ym].reduce((s, i) => s + (Number(i.valor) || 0), 0), itens: porMes[ym],
    }));
  }, [dados.investimentos]);

  function abrirNovo() { setEditId(null); setValorStr(''); setAtivo(''); setData(hojeISO()); setRecente(false); setModal(true); }
  function abrirEditar(inv) { setEditId(inv.id); setValorStr(num(inv.valor)); setAtivo(inv.ativo || ''); setData(inv.data); setRecente(!!inv.recente); setModal(true); }
  function salvar() {
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 110 }}>
        ...
        <View style={{ backgroundColor: C.primarySoft, borderRadius: 14, padding: 12, marginBottom: 16, flexDirection: 'row', gap: 8 }}>
          ...
          <Text style={{ color: C.muted, fontSize: 12, flex: 1, lineHeight: 17 }}>
            <Text style={{ color: C.text, fontFamily: F.bold }}>Recente</Text> desconta do saldo do mês (saiu da conta agora). <Text style={{ color: C.text, fontFamily: F.bold }}>Histórico</Text> só soma no patrimônio.
          </Text>
        </View>
              ...
              {g.itens.map((inv) => (
                <TouchableOpacity key={inv.id} activeOpacity={0.8} onPress={() => abrirEditar(inv)}
                  ...
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    ...
                    <Text style={{ color: C.muted, fontSize: 12 }}>{diaMesExtenso(inv.data)} · {inv.recente ? 'recente' : 'histórico'}</Text>
                  </View>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, justifyContent: 'flex-end' }}>
          ...
          <ScrollView style={{ maxHeight: '90%' }} keyboardShouldPersistTaps="handled"
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <Text style={{ color: C.text, fontFamily: F.bold, fontSize: 18 }}>{editId ? 'Editar aporte' : 'Novo aporte'}</Text>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: C.cardSoft, borderRadius: 14, paddingHorizontal: 16, marginBottom: 16 }}>
              ...
              <TextInput value={valorStr} onChangeText={setValorStr} keyboardType="numeric" placeholder="0,00" placeholderTextColor={C.mutedDim}
            ...
            <TextInput value={ativo} onChangeText={setAtivo} placeholder="Ex: Tesouro Selic, HGLG11, CDB..." placeholderTextColor={C.mutedDim}
            ...
            <TouchableOpacity onPress={() => navigation.navigate('SelecionarData', { data, onPick: (d) => setData(d) })}
              ...
              <Text style={{ color: C.text, fontFamily: F.reg, fontSize: 15 }}>{data === hojeISO() ? 'Hoje' : diaMesExtenso(data)}</Text>
            </TouchableOpacity>

            <Text style={{ color: C.muted, fontSize: 13, marginBottom: 8 }}>Tipo de aporte</Text>
            <View style={{ gap: 10, marginBottom: 20 }}>
              <OpcaoTipo ativo={recente} onPress={() => setRecente(true)} icone="arrow-top-right" titulo="Recente" sub="Saiu da conta agora — desconta do saldo deste mês" />
              <OpcaoTipo ativo={!recente} onPress={() => setRecente(false)} icone="history" titulo="Histórico" sub="Já investido antes — só soma no patrimônio" />
            </View>
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85}
      style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: 14,
        backgroundColor: ativo ? C.primarySoft : C.cardSoft, borderWidth: 1.5, borderColor: ativo ? C.primary : 'transparent' }}>
